...
export default {
  ...
  methods: {
    ...
    bindPickerChange_categroy (e) {
      // 这两个函数纯属写的脑子有问题,,,,
      console.log(
        '选中的值为：' + this.options.category_list[e.mp.detail.value]
      );
    ...
    getClubs () {
      let fly = new Fly();
      fly
        .get('http://sz.shuhelper.cn/api/ZuZhi/ZuZXX/GetAllZuZXX', {
        ...
        .then(response => {
          for (let item of response.data.data.zuzxx) {
            let club = {
              ...
              logo: item.ZuZLogo,
              logo_href:
                'http://www.sz.shu.edu.cn/Resources/ZuZTX/' + item.ZuZLogo,
